 ...
 */
import { Canvas, Coordinate, Geometry, INTERNAL_LAYER_PREFIX, LineString, MapboxUtil, Marker, Point, PointExtent, StringUtil, Util, VectorLayer, animation, renderer } from 'maptalks';
...
ClusterLayer.registerRenderer('canvas', function (_maptalks$renderer$Ve) {
    ...
    function _class(layer) {
        ...
        if (_this2.layer.options['interact']) {
            ...
            var _id = INTERNAL_LAYER_PREFIX + '_markercluster_' + layerid + '_spreadoutLayer';
    ...
    _class.prototype._calculateTo = function _calculateTo(center, index, count) {
        ...
        var to = map.containerPointToCoordinate(new Point(x, y));
        return to;
    };
    ...
    _class.prototype._createline = function _createline(from, to, marker) {
        ...
        function play() {
            if (!line) {
                ...
                sprite = new Marker(from, opt).addTo(layer);
            } else {
